...
public class Smo_Size_Change : MonoBehaviour
{
    ...
    [SerializeField] private float Size2;
    [SerializeField] private float Size3;
    [SerializeField] private float Size4;
    [SerializeField] private float Size5;
    [SerializeField] private float Size6;
    [SerializeField] private float Size7;
    ...
    [SerializeField] ParticleSystemRenderer RedParticle;
    private float size;

    private void Start()
    {
        Particle.SetActive(false);
        size = RedParticle.maxParticleSize;
    }
    ...
    void Update()
    {
        ...
        if (StartOut <= DisZ && DisZ < StartIn && Hit.hit)
        {
            ...
            SizeChange(RedParticle, Size1);

            if (StartOut <= DisZ && DisZ < StartIn1)
            {
                SizeChange(RedParticle, Size2);
            }
            if (StartOut <= DisZ && DisZ < StartIn2)
            {
                SizeChange(RedParticle, Size3);
            }
            if (StartOut <= DisZ && DisZ < StartIn3)
            {
                SizeChange(RedParticle, Size4);
            }
            if (StartOut <= DisZ && DisZ < StartIn4)
            {
                SizeChange(RedParticle, Size5);
            }
            if (StartOut <= DisZ && DisZ < StartIn5)
            {
                SizeChange(RedParticle, Size6);
            }
            if (StartOut <= DisZ && DisZ < StartIn6)
            {
                SizeChange(RedParticle, Size7);
            }
        }
    ...
    private void SizeChange(ParticleSystemRenderer particle, float size)
    {

        particle.maxParticleSize = size;
    }
