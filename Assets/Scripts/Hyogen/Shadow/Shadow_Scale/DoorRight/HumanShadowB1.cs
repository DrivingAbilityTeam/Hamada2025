...
public class HumanShadowB1 : MonoBehaviour
{
    ...
    private float DisZ;
    private float rad;
    private float zvalue = 0.912f;//影の初期位置
    private float dx = 0.00095f;
    private float dy = 0.00095f;
    private float dz = 0.00095f;
    private bool human;
    ...
    void Start()
    {
        ...
        ShadowT.transform.localPosition = new Vector3(0.457f, 1.1137f, 0.912f);//影を初期位置に戻す
    }
        ...
        if (human) {
             ...
             if (StartOut >= DisZ && DisZ > StartIn)//後方からハザードの接近を検知
             {
                ...
                Debug.Log("Start!!!!");
                    //影を前方に動かす
                    zvalue += 0.0018f;
                    Vector3 pos = new Vector3(0.457f, 1.1137f, zvalue);
                    ...
                    if (zvalue >= 1.08f)//
                    {
                        ...
                        if (size.y > 0.02f)//一定のサイズを超えたら元に戻す
                        {
